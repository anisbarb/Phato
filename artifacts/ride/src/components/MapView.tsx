...
function getTileUrl(): string {
  const style = localStorage.getItem("phato_map_style") ?? "standard";
  return MAP_TILE_URLS[style] ?? MAP_TILE_URLS.standard;
}

function TileLayerSwitcher() {
  const [url, setUrl] = useState(getTileUrl);
  useEffect(() => {
    const handler = (e: Event) => {
      const id = (e as CustomEvent).detail as string;
      setUrl(MAP_TILE_URLS[id] ?? MAP_TILE_URLS.standard);
    };
    window.addEventListener("phato_map_style_changed", handler);
    return () => window.removeEventListener("phato_map_style_changed", handler);
  }, []);
  return (
    <TileLayer
      key={url}
