...
export default function CorridorPulse({ stats, destination, matchCount = 0, route, asDriver, compact }: Props) {
  const isLive = stats.freshMs < 8000;
  const activityDot =
    stats.activity === "peak"   ? "bg-amber-400"
    : stats.activity === "quiet" ? "bg-slate-400"
    : "bg-emerald-400";

  const activityText =
    stats.activity === "peak"   ? "Busy right now"
    : stats.activity === "quiet" ? "Quiet — few vehicles"
    : "Active";
  ...
  // ── Compact single-row version for driver dashboard ──
  if (compact) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-[14px] border border-[var(--color-line)] bg-white">
        <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${activityDot} ${isLive ? "live-blink" : ""}`} />
        <span className="flex-1 min-w-0 text-[12px] font-medium text-[var(--color-ink)] truncate">
          {stats.movingCount > 0 ? `${stats.movingCount} moving` : `${stats.totalActive} active`}
          <span className="text-[var(--color-ink-mute)] font-normal"> · {activityText}</span>
        </span>
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-center">
            <span className="text-[13px] font-bold text-[var(--color-ink)] tabular-nums">{stats.silcharBound}</span>
            <span className="ml-1 text-[10px] text-[var(--color-ink-mute)]">→ SC</span>
          </div>
          <div className="text-center">
            <span className="text-[13px] font-bold text-[var(--color-ink)] tabular-nums">{stats.hailakandiBound}</span>
            <span className="ml-1 text-[10px] text-[var(--color-ink-mute)]">→ HK</span>
          </div>
          {destination && matchCount > 0 ? (
            <span className="rounded-full bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-0.5">{matchCount} on route</span>
          ) : null}
          {isLive ? (
            <div className="flex items-center gap-0.5 rounded-full bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-600 shrink-0">
  ...
  // ── Full version (passenger / driver without compact) ──
  return (
    <div className="soft-card overflow-hidden">
      ...
      {destination ? (
        <div className="flex items-center gap-3 border-t border-[var(--color-line)] px-3.5 py-3">
          <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[12px] font-bold ${matchCount > 0 ? "bg-brand text-white shadow-[0_2px_8px_rgba(37,99,235,0.30)]" : "bg-[var(--color-surface-2)] text-[var(--color-ink-mute)]"}`}>
            {matchCount}
          </div>
          <div className="min-w-0 flex-1">
          ...
          {route ? (
            <div className="shrink-0 text-right">
              <div className="text-[16px] font-bold text-[var(--color-ink)] leading-none">{formatDuration(route.durationSeconds)}</div>
              <div className="mt-0.5 text-[10px] text-[var(--color-ink-mute)]">{formatDistance(route.distanceMeters)}</div>
            </div>
          ) : null}
        </div>
      ) : null}
